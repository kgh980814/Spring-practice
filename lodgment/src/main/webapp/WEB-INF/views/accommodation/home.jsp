...
<form id="form-search-accos">
	<div class="row bg-info m-0" style="height:13vh;">
						...
						<ul class="list-group list-group-flush">
							...
							<li class="list-group-item py-3">
								...
								<div class="btn-toolbar row g-2" role="toolbar" aria-label="Toolbar with button groups">
									...
									<div class="col">
										<button type="button" id="btn-apply" class="btn btn-info w-100">적용</button>
									</div>
								</div>
							</li>
...
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=34551f66b723262141fe6f5b04652c38&libraries=services"></script>
	...
	function refreshLocation() {
		...
		if (navigator.geolocation) {
			navigator.geolocation.getCurrentPosition(function(position) {
				/* console.log("[DEG]"+displayCenterInfo);
				console.log("[DEG]eeeeeeee"+displayCurrentInfo); */
				...
				searchCurrentAddrFromCoords(map.getCenter(),displayCurrentInfo);
 ...
 */
	...
	kakao.maps.event.addListener(map, 'click', function(mouseEvent) {
	    searchDetailAddrFromCoords(mouseEvent.latLng, function(result, status) {
	        if (status === kakao.maps.services.Status.OK) {
	            ...
	            // 마커를 클릭한 위치에 표시합니다 
	            myLocationMarker.setPosition(mouseEvent.latLng);
	            myLocationMarker.setMap(map);
	...
	function searchCurrentAddrFromCoords(coords, callback) {
	    // 좌표로 현재  주소 정보를 요청합니다
	...
	// 화면 우측상단에 지도 중심좌표에 대한 주소정보를 표출하는 함수입니다
	function displayCurrentInfo(result, status) {
	    if (status === kakao.maps.services.Status.OK) {
	        var currentinfo = document.getElementById('home-current-location-address');
	        for(var i = 0; i < result.length; i++) {
	            // 행정동의 region_type 값은 'H' 이므로
	            if (result[i].region_type === 'H') {
	                currentinfo.innerHTML = result[i].address_name;
	                break;
	            }
	        }
	    }    
	...
	function displayCenterInfo(result, status) {
	    if (status === kakao.maps.services.Status.OK) {
	        var infoDiv = document.getElementById('modal-current-location-address');
	        for(var i = 0; i < result.length; i++) {
	            // 행정동의 region_type 값은 'H' 이므로
	            if (result[i].region_type === 'H') {
 ...
 */
 	...
 	function searchAccos() {
		...
		$.getJSON("/accommodations", queryString).done(function(accos) {
			if (accos.length === 0) {
				let content = `
					<p class="my-5 text-">조회된 결과가 없습니다.</p>
				`;
				$div.append(content);
			} else {
				let count = 0;
 ...
 */
	...
	// 지역을 변경했을 때 숙소 재검색 후 화면 갱신, 지도  변경
